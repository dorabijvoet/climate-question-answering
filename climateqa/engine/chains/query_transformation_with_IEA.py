...
class QueryAnalysis(BaseModel):
    ...
    sources: List[Literal["IPCC", "IPBES", "IPOS","OpenAlex", "IEA"]] = Field(
        ...,
        description="""
            ...
            - IEA is for questions about energy: for example the energy transition, electricity, renewables, carbon capture, utilisation and storage, decarbonization, fuels, energy efficiency in buildings, energy in transport, etc.

            If you choose IEA ALWAYS choose IPCC as well.
        """,
    )
...
def make_query_transform_node(llm):
    ...
    def transform_query(state):
        ...
        for question in new_state["questions"]:
            ...
            # Always use IPCC documents as well if IEA is in the sources
            if "IEA" in analysis_output["sources"] and "IPCC" not in analysis_output["sources"]:
                analysis_output["sources"].append("IPCC")
